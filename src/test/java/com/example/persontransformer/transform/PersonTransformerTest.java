...
class PersonTransformerTest {
    ...
    @Test
    void transform_withNullRaceAndEthnicity_mapsNullsToPersonCorrectly() {
        PersonEvent event = new PersonEvent("ext-1", "Jane", "Doe", "jane@example.com");
        Person person = transformer.transform(event);
    ...
    @Test
    void transform_withRaceAndEthnicityWithWhitespace_trimsFields() {
        PersonEvent event = new PersonEvent("ext-1", "Jane", "Doe", "jane@example.com",
                "  Black  ", "  Hispanic  ", null, null);
        Person person = transformer.transform(event);

        assertThat(person.getRace()).isEqualTo("Black");
    ...
    @Test
    void applyToExisting_withRaceAndEthnicity_updatesRaceAndEthnicity() {
        Person existing = new Person("mongo-id", "ext-1", "Old", "Name", "old@x.com", null);
